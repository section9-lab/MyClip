...
Bei neu aufbereiteten Seiten enthält `source_ids` die tatsächlichen IDs der im Text zitierten Aufnahmen. Referenzaufnahmen des Pakets stehen getrennt in `context_source_ids`; sie belegen nicht jede Aussage. MCP stellt beide Listen und die Beobachtungszeit bereit. Vorhandene Notizen bleiben lesbar und übernehmen diese Regeln bei erneuter Aufbereitung. Ein Upgrade schreibt sie nicht gesammelt um.

Die Suche in App und MCP verwendet dieselbe SQLite-FTS5-Rangfolge. Suchwörter dürfen mit beliebigen Einzelbegriffen übereinstimmen. Vollständige Ausdrücke in Titel und Text werden zuerst berücksichtigt, danach BM25-Relevanz und Dateiänderungszeit. Wörtliche Teilzeichenfolgen bleiben für Chinesisch und Satzzeichen verfügbar. Eine leere Suche zeigt zuletzt bearbeitete Notizen. Suchvorschauen bevorzugen Absätze mit dem vollständigen Ausdruck oder mehr unterschiedlichen Suchwörtern.

Die MCP-Suche liefert bis zu drei `matches` pro Notiz. Jeder Treffer enthält einen Originalauszug von höchstens 1.600 Zeichen, `path`, `revision`, `startOffset` und den exklusiven `endOffset`. Offsets zählen Zeichen im Textkörper ohne YAML-Metadaten am Dateianfang. Die `sourceIDs` eines Treffers enthalten ausschließlich explizite Quellenzitate dieses Absatzes, die auch zu den Quellen der Notiz gehören. Ohne Zitat im Absatz geben `sourceScope: "document"` und `documentSourceIDs` die Dokumentherkunft an, ohne sie als Beleg für diesen Absatz zu behandeln. `summary` und `summaryOffset` bleiben aus Kompatibilitätsgründen verfügbar. Übergib für `read_memory` den Wert `match.startOffset` als `offset` und `match.revision` als `revision`. Eine veraltete Revision wird abgelehnt, damit eine Bearbeitung den Lesezugriff nicht unbemerkt auf anderen Inhalt umleitet.

Bei MCP `search_memories` ist `since` inklusiv und `until` exklusiv; beide akzeptieren ISO-8601-Zeitstempel. `timeField: "updated"` behält die standardmäßige Filterung nach Dateiänderungszeit bei. `timeField: "captured"` filtert den Zeitpunkt einer zitierten Aufnahme. Wenn zusätzlich `app` angegeben ist, muss dieselbe Aufnahme beide Filter erfüllen. `timeField: "event"` filtert explizit erfasste Ereignisintervalle, die sich mit dem Suchzeitraum überschneiden, oder darin liegende Zeitpunkte. Suchwörter und `app` müssen zum selben Ereignisabsatz passen, nicht zu voneinander unabhängigen Stellen der Notiz. Unbekannte Ereignisdaten werden ausgeschlossen. Aufnahme- und App-Filter benötigen verfügbare Aufnahmemetadaten; Ereignisdaten und Quell-IDs lassen sich allein aus Markdown wiederherstellen.
