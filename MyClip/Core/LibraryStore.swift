...
public actor LibraryStore {
    ...
    public init(root: URL) throws {
        ...
        guard version <= 6 else { throw LibraryError.database("资料库由更新的 MyClip 创建，请升级应用。") }
        try database.script("""
    ...
    public func nextImageForTextIndex() throws -> (id: String, url: URL)? {
        guard let id = try database.run("SELECT id FROM images WHERE available=1 AND id NOT IN (SELECT image_id FROM image_text) ORDER BY rowid LIMIT 1").first?["id"] else { return nil }
    ...
    public func rebuildSearchIndex() throws {
        ...
        try database.transaction {
            ...
            for row in try database.run("SELECT * FROM entries") {
                ...
                try database.run("INSERT INTO entry_search VALUES(?,?,?,?)", [item.id.uuidString, item.title, item.body, Self.tokens(item.title + " " + item.body)])
            }
    ...
    public func snapshot(query: String = "") throws -> LibrarySnapshot {
        try synchronizeMemoryFiles()
        ...
        let captureFilter = term.isEmpty ? "" : """
            WHERE instr(lower(c.app_name || ' ' || c.window_title), ?) > 0 OR c.image_id IN (
        ...
        result.captures = try database.run("""
            ...
            \(captureFilter) ORDER BY c.captured_at DESC, c.rowid DESC LIMIT 500
            """, term.isEmpty ? [] : [term, captureMatch, term]).map(capture)
        var entrySQL = "SELECT * FROM entries"
        var arguments: [String?] = []
        if !term.isEmpty {
            let match = Self.tokens(term).split(separator: " ").map { "\"\($0.replacingOccurrences(of: "\"", with: "\"\""))\"" }.joined(separator: " AND ")
            entrySQL += """
                 WHERE id IN (
                    SELECT id FROM entry_search WHERE entry_search MATCH ?
                    UNION SELECT id FROM entry_search WHERE instr(lower(title),?)>0 OR instr(lower(body),?)>0
                 )
                """
            arguments = [match.isEmpty ? "\"\(term.replacingOccurrences(of: "\"", with: "\"\""))\"" : match, term, term]
        }
        result.entries = try database.run(entrySQL + " ORDER BY updated_at DESC,id", arguments).map(entry)
    ...
    public func finishMemoryEditing(jobID: UUID, previousRevisions: [UUID: Int]) throws -> Int {
        ...
        try database.transaction {
            for entry in changed {
                let sources = Array(Set(entry.sourceIDs + task.sourceIDs)).sorted { $0.uuidString < $1.uuidString }
    ...
    func entry(_ row: [String: String]) throws -> KnowledgeEntry {
        ...
        let sources = try MemoryDocument(text).sourceIDs
        return KnowledgeEntry(id: id, kind: .memory, title: row["title"] ?? "", body: body,
                              ...
                              agent: agent, sourceIDs: sources, fileURL: try memoryURL(id),
    ...
    func saveEntry(id: UUID, kind: KnowledgeKind, title: String, body: String, revision: Int,
                           ...
                           extraMetadata: String? = nil) throws -> URL {
        ...
        let retainedMetadata = try oldHistory.map { try MemoryDocument(String(contentsOf: root.appendingPathComponent($0), encoding: .utf8)).extraMetadata } ?? ""
        let date = updatedAt ?? Date()
        let directory = root.appendingPathComponent("Entries/\(id.uuidString)")
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        let path = "Entries/\(id.uuidString)/\(revision).md"
        let url = root.appendingPathComponent(path)
        let text = try MemoryDocument.encode(id: id, title: title, body: body, revision: revision, agent: agent, sourceIDs: sourceIDs, path: memoryPath, updatedAt: date, extraMetadata: extraMetadata ?? retainedMetadata)
        ...
        do {
            ...
            try database.run("INSERT INTO entry_search(id,title,body,terms) VALUES(?,?,?,?)", [id.uuidString, title, body, Self.tokens(title + " " + body)])
