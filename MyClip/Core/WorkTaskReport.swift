...
public struct WorkTaskReport: Sendable {
    ...
    public var summary: String { "新增 \(added) 项，完成 \(completed.count) 项；\(isCurrent ? "目前" : "期末")进行中 \(doing.count) 项，待办 \(todo.count) 项。" }
    ...
    public init(period: WorkTaskReportPeriod, containing date: Date, tasks: [WorkTask], events: [WorkTaskEvent], now: Date = Date(), calendar: Calendar = .current) {
        ...
        formatter.dateFormat = period == .month ? "yyyy年M月" : "yyyy年M月d日"
    ...
    public var document: WorkTaskReportDocument {
        let current = period == .day ? "今日" : period == .week ? "本周" : "本月"
        let next = period == .day ? "明日" : period == .week ? "下周" : "下月"
        ...
        return WorkTaskReportDocument(period: period, interval: interval, dateTitle: dateTitle, sections: [
            .init(id: "outcomes", title: "一、\(current)工作与成果", projects: projects(completed)),
            .init(id: "progress", title: "二、进行中的工作与问题", projects: projects(doing + waiting.filter { !doingIDs.contains($0.id) })),
            .init(id: "plans", title: "三、\(next)工作计划", projects: projects(doing + todo, planning: true))
        ])
    }
...
public struct WorkTaskReportDocument: Codable, Equatable, Identifiable, Sendable {
    ...
    public struct Project: Codable, Equatable, Identifiable, Sendable {
        ...
        public var progress: String {
            [(WorkTaskStatus.done, "完成"), (.doing, "推进中"), (.todo, "待办")].compactMap { status, title in
                let count = items.filter { $0.status == status }.count
                return count == 0 ? nil : "\(title) \(count) 项"
            }.joined(separator: " · ")
        }
    }
    ...
    public var title: String { "工作\(period.title)" }
    ...
    public var markdown: String {
        ...
        for section in sections {
            lines += ["", "## \(section.title)"]
            if section.projects.isEmpty { lines += ["", "暂无记录。"] }
...
extension LibraryStore {
    public func savedWorkTaskReport(for report: WorkTaskReport) throws -> WorkTaskReportDocument? {
        ...
        guard document.id == report.document.id else { throw LibraryError.invalidResult("报告日期与草稿不一致") }
        return document
    }
