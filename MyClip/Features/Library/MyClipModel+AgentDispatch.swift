...
extension MyClipModel {
            ...
            try await withAgentSession(agent) { client, session in
                do {
                    let result = try await trackedPrompt(client, agent: agent, session: session,
                        text: KnowledgeComposer.coldStartPrompt(files: files.map { (label: $0.label, text: $0.text) }), images: [], jobID: nil)
            ...
            do {
                ...
                try await withAgentSession(job.agent) { client, session in
                    ...
                    let result = try await trackedPrompt(client, agent: job.agent, session: session,
                        text: KnowledgeComposer.organizeBatchPrompt(inputs: inputs, handoff: handoff, previousAttempt: job.attempts > 1 ? job.error : nil, tasks: existingTasks),
    ...
    private func dream(_ job: ClipJob) async {
        ...
        do {
            ...
            let turns = KnowledgeComposer.dreamTurns(plan: plan, handoff: try await store.organizationHandoff())
