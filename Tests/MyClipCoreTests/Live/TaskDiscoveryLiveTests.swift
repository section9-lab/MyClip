...
final class TaskDiscoveryLiveTests: XCTestCase {
    func testAgentFindsTaskFromMemoryWithoutEditingIt() async throws {
        ...
        let response = try await client.prompt(sessionID: session, text: TaskPrompt.discoveryPrompt(memories: [updated], tasks: []), images: [])
        XCTAssertEqual(response.stopReason, "end_turn")
        let drafts = try TaskResponse.parse(response.text)
