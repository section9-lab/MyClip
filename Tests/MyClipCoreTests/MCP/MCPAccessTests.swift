...
final class MCPAccessTests: XCTestCase {
    ...
    func testInitializeCarriesProfileAndFocusDigestOnceTheyAreWritten() async throws {
        ...
        XCTAssertEqual(seeded, MCPPrompt.instructions, "Seeded templates add nothing")
        ...
        XCTAssertTrue(text.hasPrefix(MCPPrompt.instructions))
        ...
        XCTAssertLessThan(text.count - MCPPrompt.instructions.count, 2 * MemoryMCP.digestFileLimit + 400)

        try MemoryMCP.setEnabled(false, in: root)
        let disabled = try await instructions(MemoryMCP(store: store))
        XCTAssertEqual(disabled, MCPPrompt.instructions, "A disabled server shares nothing at connection time")
    }
