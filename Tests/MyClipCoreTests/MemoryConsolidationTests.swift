...
final class MemoryConsolidationTests: XCTestCase {
    ...
    func testPlanCoversChangedPagesNeighboursAndAPatrolOfOldPages() async throws {
        ...
        let turns = KnowledgeComposer.dreamTurns(plan: plan, handoff: nil)
    ...
    func testMisfiledPagesLeadTheScopeWithinTheLimit() async throws {
        ...
        let prompt = KnowledgeComposer.consolidationPrompt(plan: plan, handoff: nil)
    ...
    func testConsolidationPromptStaysWithinScopeAndAddsNoFacts() {
        ...
        let prompt = KnowledgeComposer.consolidationPrompt(plan: plan, handoff: "整理提示：……")
    ...
    func testOrganizingPromptNamesNewFoldersAliasesAndDailySplit() {
        let prompt = KnowledgeComposer.filePrompt(inputs: [])
        ...
        XCTAssertTrue(KnowledgeComposer.coldStartPrompt(files: []).contains("人物放 Wiki/People"))
    }

    func testOrganizingPromptIsGroupedByWhatTheAgentDecides() {
        let prompt = KnowledgeComposer.organizeBatchPrompt(inputs: [], handoff: "交接", tasks: [])
        ...
        let dream = KnowledgeComposer.consolidationPrompt(plan: ConsolidationPlan(misfiled: [], changed: ["Now.md"], neighbours: []), handoff: nil)
        XCTAssertTrue(dream.contains("【每类页面】") && !dream.contains("【返回】"), "Reviews share the rules but return no task JSON")
        XCTAssertTrue(KnowledgeComposer.coldStartPrompt(files: []).contains("本次资料是文件而不是截图"), "Cold start maps screenshot wording to its files")
    }
    ...
    func testEntityPagesUseDatedFactLinesAndEditsStayLocal() {
        ...
        let dream = KnowledgeComposer.consolidationPrompt(plan: ConsolidationPlan(misfiled: [], changed: ["Daily/2026/09/2026-09-20.md"], neighbours: []), handoff: nil)
